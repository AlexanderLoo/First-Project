...
public class Reposition : MonoBehaviour {
	...
	void OnTriggerExit2D(Collider2D other){
		...
		if (other.CompareTag("Player")) {
			...
			if (other.transform.position.x > screenSize.maxInX || other.transform.position.x < -screenSize.maxInX) {
			...
			if (other.transform.position.y > screenSize.maxInY || other.transform.position.y < -screenSize.maxInY) {
